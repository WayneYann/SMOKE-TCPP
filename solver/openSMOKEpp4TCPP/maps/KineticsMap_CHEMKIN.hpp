...
namespace OpenSMOKE
{
	...
	//Ali on Sep 2017
	void KineticsMap_CHEMKIN::DerivativesOfReactionRates(const double* c, Eigen::MatrixXd* dR_over_dC)
	{
		const double ZERO_DER = std::sqrt(OPENSMOKE_TINY_FLOAT);
		const double ETA2 = std::sqrt(OPENSMOKE_MACH_EPS_DOUBLE);			
		const double TOLR = 100. * OPENSMOKE_MACH_EPS_FLOAT;
		...
		ReactionRates(c);
		//FormationRates(R_original.GetHandle());
		OpenSMOKEVectorDouble rrNet_original(this->number_of_reactions_);
		OpenSMOKEVectorDouble rrNet_plus(this->number_of_reactions_);	
		GiveMeReactionRates(rrNet_original.GetHandle());
		...
		for(unsigned int kd=1;kd<=this->number_of_species_;kd++)
		{
			if(c[kd-1] <= 1.e-100)
			{
				...
				ReactionRates(c_plus.GetHandle());
				//FormationRates(R_plus.GetHandle());
				GiveMeReactionRates(rrNet_plus.GetHandle());

				//for(unsigned int j=1;j<=this->number_of_species_;j++)
					//(*dR_over_dC)(j-1,kd-1) = (R_plus[j]-R_original[j]) * udc;
				for(unsigned int j=1;j<=this->number_of_reactions_;j++)
					(*dR_over_dC)(j-1,kd-1) = (rrNet_plus[j]-rrNet_original[j]) * udc;
