...
//Ali
std::vector<double>  CharacteristicChemicalTimes::FromFormationRatesAll(const double T, const double P, const OpenSMOKE::OpenSMOKEVectorDouble& omega)
{
	const double dC_over_dt_threshold = 1e-16;
	...
	for(unsigned int j=0;j<ns_;j++)
	{
		...
		const double tau_local = (dC_over_dt > dC_over_dt_threshold) ? c_[j+1]/dC_over_dt : 0.;
		tau[j]=tau_local;
		if (tau[j] == 0.)	tau[j] = tauChem_threshold_;
		tau[j] = min(tau[j], tauChem_threshold_);
		// Selection of maximum characteristic time
		//tau = (tau_local > tau) ? tau_local : tau;
	}
...
//Ali
std::vector<double> CharacteristicChemicalTimes::FromEigenValueAnalysisAll(const double T, const double P, const OpenSMOKE::OpenSMOKEVectorDouble& omega)
{
	if (T <= tauChem_temperature_threshold_)
		{
			std::vector<double> tau;
			tau.resize(ns_);
			for(unsigned int i=0;i<n_;i++) tau[i]=tauChem_threshold_;
			return tau;
		}
	else
	{	
		...
		std::vector<double> tau;
		tau.resize(ns_);
		for(unsigned int i=0;i<ns_;i++) tau[i]=1e-16;
		for(unsigned int i=0;i<ns_;i++)
		{
			if (lambda_real_[i] > 0. && lambda_real_[i] < 1e16)
				tau[i] = 1./lambda_real_[i];
		}
		return tau;
	}
}
